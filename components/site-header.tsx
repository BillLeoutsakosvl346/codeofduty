/* eslint-disable @next/next/no-html-link-for-pages -- Hard navigations avoid stalled Vinext client transitions on Sites. */
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#070908]/92 px-5 py-4 backdrop-blur-xl md:px-10">
      <div className="mx-auto flex max-w-[1480px] items-center justify-between gap-4">
        <a href="/" className="flex items-center gap-3" aria-label="Code of Duty home">
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {links.map(({ href, label, id, icon: Icon }) => <a key={href} href={href} aria-current={active === id ? 'page' : undefined} className={`flex items-center gap-2 px-3 py-2 text-[10px] font-black uppercase tracking-[0.15em] transition ${active === id ? 'bg-[#b8ff38]/10 text-[#b8ff38]' : 'text-white/40 hover:bg-white/5 hover:text-white'}`}><Icon size={14} />{label}</a>)}
        </nav>
      ...
      <nav className="mx-auto mt-3 grid max-w-[1480px] grid-cols-3 border border-white/10 md:hidden">
        {links.map(({ href, label, id }) => <a key={href} href={href} aria-current={active === id ? 'page' : undefined} className={`py-2 text-center text-[9px] font-black uppercase tracking-wider ${active === id ? 'bg-[#b8ff38]/10 text-[#b8ff38]' : 'text-white/40'}`}>{label}</a>)}
      </nav>
    </header>
  );
}
